...
function refresh_highlight(last) {
  for (let i = 0; i < HeadingFlag.length; ++i) {
    ...
    if (HeadingFlag[i]) {
      document
        .querySelector(`.toc-panel li[headingIdx="${i}"]`)
...
function refresh_fold(last) {
  // console.log(last);
  for (let i = 0; i < HeadingFlag.length; ++i) {
    const cur_toc_item = document.querySelector(
      `.toc-panel li[headingIdx="${i}"]`,
    );
    ...
    if (HeadingFlag[i] || i === last) // 展开双亲节点
    {
      ...
      if (
        ...
        cur_toc_item.parentElement.parentElement &&
        ...
        while (
          sub_item &&
          sub_item.parentElement &&
          sub_item.parentElement.parentElement &&
          sub_item.parentElement.parentElement.previousElementSibling &&
          sub_item.parentElement.parentElement.previousElementSibling.tagName ==
            "BUTTON"
        ) {
